...
#Regexs for box office values
box_office_form_one = r'\$\d+\.?\d*\s*[mb]illion'
box_office_form_two = form_two = r'\$\s*\d{1,3}(?:[,\.]\d{3})+(?!\s[mb]illion)'
...
def movie_data(wiki_data, kaggle_data, ratings_data):
    ...
    #Merge column names
        ...
        return movie
    #create list of clean movies and dataframe
    clean_movies=[clean_movie(movie) for movie in wiki_movies]
    wiki_movies_df=pd.DataFrame(clean_movies)
    
    #IMDb IDs and drop duplicate
    ...
    #Clean Kaggle Data
    #Drop adult films due to unrecoverable scrambled data
    kaggle_metadata = kaggle_metadata[kaggle_metadata['adult'] == 'False'].drop('adult',axis='columns')
    
    #Assign back to video
    kaggle_metadata['video'] = kaggle_metadata['video'] == 'True'
    
    #Convert the rest (numeric columns) with to_numeric() method from Pandas. Errors='raise'
    kaggle_metadata['budget'] = kaggle_metadata['budget'].astype(int)
    kaggle_metadata['id'] = pd.to_numeric(kaggle_metadata['id'], errors='raise')
    kaggle_metadata['popularity'] = pd.to_numeric(kaggle_metadata['popularity'], errors='raise')
    
    #Convert release_date to datetime with Pandas function to_datetime()
    kaggle_metadata['release_date'] = pd.to_datetime(kaggle_metadata['release_date'])
    
    #Convert original unix timestamp to datetime data type
    ratings['timestamp']=pd.to_datetime(ratings['timestamp'], unit='s')
    
    #Merge Wikipedia and Kaggle Data
    #Identify duplicate columns and left merge
    ...
    #Put it all together
    #Drop title_wiki, release_date_wiki, Language, Production company(s)
    movies_df.drop(columns=['title_wiki','release_date_wiki','Language','Production company(s)'], inplace=True)
    
    #make function to fill in missing data and drop redundant column
    ...
    #UD columns with only one value, convert lists to tuples
    try:
    ...
    #Include rating data
    # use groupby to take count of each group
    ...
    #Use left merge to keep everything in mvoies_df
...
movie_data('wikipedia-movies.json', 'movies_metadata.csv', 'ratings.csv')